...
### 文章分享帖风格规范（公众号文章同步到星球）

发布公众号文章分享帖时，**必须**采用「链接在前、简短介绍在后」的固定格式，与历史帖子保持统一：
...
**格式要点：**
1. **第一行直接贴公众号链接**（知识星球会自动渲染成带标题的卡片，无需手动写标题）
2. **空一行**
3. **再写 1-2 句简短介绍**，点出痛点或核心内容，不要长篇大论

**示例：**
```
https://mp.weixin.qq.com/s/dcbkFPtnBbmsvUJc0jIHeA

魔改版 vnpy 怎么从 4.1 安全升到 4.4？这篇记录了完整过程：通知系统重构（邮件+微信双推送）、CTP 拒单状态修复（策略终于能感知「被拒」），以及保住本地定制的升级三步法。
```

**禁止：**
- ❌ 把介绍写在前面、链接写在后面（打乱阅读顺序）
- ❌ 写成多段长文（星球是短内容平台，长文另用「发布文章」功能）
- ❌ 手动重复写文章标题（链接卡片已自动展示）
