...
/**
 * Analyze a cover image using Qwen Visual Understanding API
 */
export async function analyzeCoverImage(
  ...
  if (!dashscopeKey) {
    console.error('Error: DASHSCOPE_API_KEY is required for cover analysis');
  ...
  try {
    // Qwen API 使用 data URI 格式传递 base64 图片
    ...
    // Enhanced error handling for Qwen API
    if (!response.ok || data.code || (data.status_code && data.status_code !== 200)) {
      ...
      console.error(`Qwen API error: ${errorMsg} (code: ${errorCode})`);
      return null;
    }

    // Qwen response format: output.choices[0].message.content[0].text
    const textContent = data.output?.choices?.[0]?.message?.content?.[0]?.text;
    if (!textContent) {
      console.error('No analysis result from Qwen');
      return null;
    }
