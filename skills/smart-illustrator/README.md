...
>
> New Excalidraw engine for hand-drawn concept diagrams. Three-tier priority: Qwen → Excalidraw → Mermaid. All diagram engines now output PNG by default. [Details →](#tri-engine-system)

![Tri-Engine Architecture](assets/dual-engine-architecture.png)

Intelligent article illustration Skill for Claude Code with **tri-engine system**: automatically selects Qwen (for creative visuals), Excalidraw (for hand-drawn diagrams), or Mermaid (for structured diagrams) based on content type.
...
Creating illustrations for articles is time-consuming: manual design takes hours, stock photos lack context, and generic AI tools don't understand article structure. Smart Illustrator combines intelligent position detection, tri-engine system (Qwen + Excalidraw + Mermaid), and cover learning to generate contextual illustrations in minutes.
...
- **Tri-Engine System**: Auto-selects Qwen, Excalidraw, or Mermaid based on content type
...
- **Multi-Platform Sizes**: YouTube, WeChat, Xiaohongshu presets
...
- **Multiple Backends**: Qwen API for creative visuals (2K resolution), Excalidraw for hand-drawn diagrams, Mermaid CLI for structured diagrams — all output PNG by default
...
- Qwen API Key (optional, for creative visuals): https://dashscope.console.aliyun.com/apiKey
...
| `--engine` | `auto` | Engine: `auto`, `qwen`, `excalidraw`, or `mermaid` |
| `--mermaid-embed` | `false` | Embed Mermaid code blocks instead of exporting PNG |
| `--platform` | `youtube` | Cover platform: `youtube`/`wechat`/`xiaohongshu`/`landscape`/`square` |
...
| `-m, --model` | Model (default: qwen-image-2.0-pro) |
...
1. **Use `pictures[]` array** - Array structure helps Qwen enter "loop execution" mode for batch generation.
...
|-------------|-----------|-------------|--------------|
| Generate Images | Default | Calls Qwen API to generate images | ✅ Yes |
| Output JSON Prompt | `--prompt-only` | Copy to Qwen Web for manual generation | ❌ No |
...
> **💡 Tip**: Batch image generation with JSON in Qwen doesn't have 100% success rate. If generation fails:
...
Generate high-CTR cover images for YouTube, WeChat, and more. Built on YouTuber best practices research.
...
|----------|--------|----------|--------|
| **1** | **Qwen** | Creative visuals (metaphors, scenes, infographics) | PNG (2K) |
...
**Selection logic:**
- Needs metaphor, emotion, or creative expression → Qwen
...
| `concept` | Excalidraw / Qwen | Abstract concepts | Hand-drawn / Center-radial |
| `comparison` | Excalidraw / Qwen | A vs B, contrasts | Hand-drawn / Left-right split |
| `data` | Qwen | Statistics, trends | Infographic style |
| `scene` | Qwen | Stories, scenarios | Narrative illustration |
| `metaphor` | Qwen | Analogies, symbols | Creative visual |
| `cover` | Qwen | Article cover | 16:9 dark tech |
...
├── LICENSE
├── scripts/
│   ├── generate-image.ts     # Qwen single image generation
│   ├── batch-generate.ts     # Qwen batch generation (2K, resume support)
...
│   ├── style-light.md        # Light style Qwen prompt (default)
│   ├── style-dark.md         # Dark style Qwen prompt
│   ├── style-minimal.md      # Minimal style Qwen prompt
...
### Qwen Engine Parameters

| Parameter | Value | Description |
|-----------|-------|-------------|
| Model | `qwen-image-2.0-pro` | Best for illustrations |
...
When using Qwen API for auto-generation:

| Model | Price | Quality |
|-------|-------|---------|
| `qwen-image-2.0-pro` | $0.134/image ≈ ¥1/image | 2K (default) |
...
- [Qwen API](https://ai.google.dev/) - Google's image generation API
