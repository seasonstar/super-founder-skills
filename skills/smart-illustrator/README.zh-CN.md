...
>
> 新增 Excalidraw 手绘概念图引擎。三级优先级：Qwen → Excalidraw → Mermaid。所有图表引擎默认输出 PNG。[查看详情 →](#三引擎系统)

![三引擎架构](assets/dual-engine-architecture.png)

Claude Code 的智能文章配图 Skill，采用**三引擎系统**：根据内容类型自动选择 Qwen（创意视觉）、Excalidraw（手绘图表）或 Mermaid（结构化图表）。
...
为文章配图很费时间：手动设计需要数小时，图库图片缺乏定制性，通用 AI 工具不理解文章结构。Smart Illustrator 结合智能位置检测、三引擎系统（Qwen + Excalidraw + Mermaid）和封面学习，几分钟内生成符合上下文的插图。
...
- Qwen API Key（可选，用于创意视觉）：https://dashscope.console.aliyun.com/apiKey
...
| `--engine` | `auto` | 引擎：`auto`、`qwen`、`excalidraw` 或 `mermaid` |
| `--mermaid-embed` | `false` | 嵌入 Mermaid 代码块而非导出 PNG |
| `--platform` | `youtube` | 封面平台：`youtube`/`wechat`/`xiaohongshu`/`landscape`/`square` |
...
1. **使用 `pictures[]` 数组** - 数组结构有助于 Qwen 进入"逐条执行"模式进行批量生成。
...
| 输出方式 | 参数 | 说明 | 需要 API |
|----------|------|------|----------|
| 直接生成图片 | 默认 | 调用 Qwen API 生成图片 | ✅ 需要 |
| 输出 JSON Prompt | `--prompt-only` | 复制到 Qwen Web 手动生成 | ❌ 不需要 |
...
> **💡 使用提示**：在 Qwen 中使用 JSON 批量生成图片并非 100% 成功率。如果遇到生成失败，可以：
> 1. 重试 1-3 次，通常能成功
> 2. 如果多次重试仍失败，将 JSON 中的 `pictures` 逐条复制到 Qwen 单独生成
...
**限制**：
- 参考图功能仅支持 Qwen API（OpenRouter 不支持多模态输入）
- 使用参考图时会自动从 OpenRouter 切换到 Qwen
- 最多 3 张参考图
...
为 YouTube、公众号等生成高点击率封面图，内置 YouTuber 最佳实践。
...
| 优先级 | 引擎 | 适用场景 | 输出 |
|--------|------|----------|------|
| **1** | **Qwen** | 创意视觉（隐喻图、场景图、信息图） | PNG (2K) |
...
| `concept` | Excalidraw / Qwen | 抽象概念 | 手绘 / 中心辐射 |
| `comparison` | Excalidraw / Qwen | A vs B、对比 | 手绘 / 左右分栏 |
| `data` | Qwen | 统计、趋势 | 信息图风格 |
| `scene` | Qwen | 故事、场景 | 叙事插画 |
| `metaphor` | Qwen | 类比、象征 | 创意视觉 |
| `cover` | Qwen | 文章封面 | 16:9 深色高对比 |
...
├── LICENSE
├── scripts/
│   ├── generate-image.ts     # Qwen 单张图片生成
│   ├── batch-generate.ts     # Qwen 批量生成（2K、断点续传）
...
│   ├── style-light.md        # 浅色风格 Qwen Prompt（默认）
│   ├── style-dark.md         # 深色风格 Qwen Prompt
│   ├── style-minimal.md      # 极简风格 Qwen Prompt
...
| 模型 | 价格 | 质量 |
|------|------|------|
| `qwen-image-2.0-pro` | $0.134/张 ≈ ¥1/张 | 2K（默认） |
...
- [Qwen API](https://ai.google.dev/) - Google 的图片生成 API
