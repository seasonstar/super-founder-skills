...
"""
Markdown 图片处理工具
自动识别、上传 Markdown 中的本地图片到无铭图床(wmimg.com)
支持本地图片和外链图片
"""
...
from wmimg_uploader import WmimgUploader
...
class MarkdownImageProcessor:
    ...
    def __init__(self, wmimg_token: str):
        self.uploader = WmimgUploader(wmimg_token)
    ...
    def upload_image(self, image_source: str, is_local: bool = True) -> str | None:
        ...
        try:
            ...
            result = self.uploader.upload(temp_path)
    ...
    def process_markdown(self, content: str, base_dir: str = None, dry_run: bool = False) -> tuple[str, list]:
        """
        处理 Markdown 内容，上传所有图片（本地和外链）到 wmimg 图床
        ...
        print(f"\n🖼️ 发现 {len(images)} 张图片")
        if local_images:
            print(f"   📦 本地图片: {len(local_images)} 张（上传到 wmimg）")
        if remote_images:
            print(f"   🔗 外链图片: {len(remote_images)} 张（转存到 wmimg）")
        ...
        for img in images:
            image_path = img['path']
            is_local = self.is_local_path(image_path)
            print(f"\n处理: {image_path}")

            if dry_run:
                src_type = "本地" if is_local else "外链"
                print(f"  🔍 预览: {src_type} -> wmimg 图床")
                ...
                if is_local:
                    ...
                    # 外链图片：下载后上传到 wmimg
                    new_url = self.upload_image(image_path, is_local=False)
...
def main():
    if len(sys.argv) < 2:
        print("用法:")
        print("  python markdown_image_uploader.py <markdown文件> [WMIMG_TOKEN]")
        print("")
        print("示例:")
        print("  python markdown_image_uploader.py ./articles/test.md")
        print("  python markdown_image_uploader.py ./articles/test.md 804|AyKazkrkLtChWgsKNURWKBWovKaBNBEsN28CPD5P")
        sys.exit(1)
    ...
    # 处理图片
    processor = MarkdownImageProcessor(token)
    ...
    if success_count > 0:
        # 输出处理后的文件
        output_file = md_file.replace('.md', '_uploaded.md')
